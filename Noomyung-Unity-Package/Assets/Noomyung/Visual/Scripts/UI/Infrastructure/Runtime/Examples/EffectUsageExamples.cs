...
namespace Noomyung.UI.Infrastructure.Runtime.Examples
{
    ...
    public class EffectUsageExamples : MonoBehaviour
    {
        ...
        public void CreateTypedEffects()
        {
            ...
            // 2. Scale Effect 생성 (3번 반복)
            var scaleEffect = new ScaleEffect(
                from: Vector3.Zero,
                to: Vector3.One,
                timing: new EffectTiming(duration: 0.5f, delay: 0.2f, direction: PlaybackDirection.Forward, repeatMode: RepeatMode.Finite, cycleCount: 3),
                easing: new EffectEasing(EasingType.EaseOut),
                axisMask: "XYZ"
            );

            // 3. Move Effect 생성 (PingPong 2번)
            var moveEffect = new MoveEffect(
                from: Vector3.Zero,
                to: new Vector3(100, 50, 0),
                timing: new EffectTiming(duration: 2f, delay: 0f, direction: PlaybackDirection.PingPong, repeatMode: RepeatMode.Finite, cycleCount: 2),
                easing: new EffectEasing(EasingType.EaseInOut),
                space: "Anchored"
            );

            // 4. Color Effect 생성 (무한 반복)
            var colorEffect = new ColorEffect(
                from: Color.White,
                to: Color.Red,
                timing: new EffectTiming(duration: 1f, delay: 0f, direction: PlaybackDirection.Forward, repeatMode: RepeatMode.Loop),
                easing: new EffectEasing(EasingType.Linear),
                targetMode: "Graphic"
            );

            // 5. Shake Effect 생성 (PingPong 무한 반복)
            var shakeEffect = new ShakeEffect(
                strength: new Vector3(10, 10, 0),
                timing: new EffectTiming(duration: 0.5f, delay: 0f, direction: PlaybackDirection.PingPong, repeatMode: RepeatMode.Loop),
                easing: new EffectEasing(EasingType.Linear),
                frequency: 15f,
                vibrato: 10
            );
